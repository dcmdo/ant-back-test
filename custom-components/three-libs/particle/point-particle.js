...
import ObjectFns from "../../global-libs/under-score/object-fns";
 ...
 * startRotation:[min,max] 粒子出生旋转
 * pivotOffset:{Number} 沿初始方向的偏移值0;
 * textureAnimation:{
 ...
 *     isPlay:{Boolen} true
 * }
 * material:{PointParticleMaterial} 材质
 * shape:{Shape}
 * emission:{
 *     rate: {Number} 10,
 ...
 * renderMode:{
 *      pivotOffset:0,
 *      algin:"view" || "init" || "speed"
 * }
 // 时间线效果
 * timeSize:[time,size,time,size,……] 数值对不能超过5个 time：0-1
 * timeRotation:[min,max]
 * timeColor:[time,color,time,color,……] 数值对不能超过5个 time：0-1
 * timeOpacity:[time,opacity,time,opacity,……] 数值对不能超过5个 time：0-1
 */
export default class PointPaticle extends THREE.Points{
    constructor(paras){
        let geometry = new THREE.BufferGeometry();
        paras = paras || {};
        paras.material = paras.material || new ThreeLibs.PointParticleMaterial();
    ...
    //----------------------------------------内部属性方法----------------------------------------
    declareVars(paras){
        this._renderView = new THREE.Vector4();
        this._viewHeight = 0;
    ...
    setAttributes(){
        ...
        let attributes = {
            time: new THREE.BufferAttribute(new Float32Array(maxCount),1),
            life: new THREE.BufferAttribute(new Float32Array(maxCount),1),
            position: new THREE.BufferAttribute(new Float32Array(maxCount3),3),
            size: new THREE.BufferAttribute(new Float32Array(maxCount),1),
            startRotation: new THREE.BufferAttribute(new Float32Array(maxCount),1),
            initDirection: new THREE.BufferAttribute(new Float32Array(maxCount3),3),
            direction:new THREE.BufferAttribute(new Float32Array(maxCount3),3),
            startColor:new THREE.BufferAttribute(new Float32Array(maxCount3),3),
            startOpacity:new THREE.BufferAttribute(new Float32Array(maxCount),1),
            textureRandomIndex:new THREE.BufferAttribute(new Int8Array(maxCount),1),
            timeRotation:new THREE.BufferAttribute(new Float32Array(maxCount),1)
        };
    ...
    initPoints(){
        ...
        this.onBeforeRender = (renderer)=>{
            renderer.getViewport(this._renderView);
            if(this._renderView.w !== this._viewHeight){
                this._viewHeight = this._renderView.w;
                this.material.viewHeight = this._viewHeight * renderer.getPixelRatio();
            }
        };
    ...
    checkLife(){
        ...
        for( let i = currentCount - 1; i > -1; i--){
            if(this._time[i] >= 1){
                ...
                this._position.splice(i3,3);
                this._startRotation.splice(i,1),
    ...
    updateParticle(delta){
        ...
        if(this._isUpdateAttribute){
            attributes = [
                ["time",this._time,Float32Array], ["position",this._position,Float32Array],
                ["life",this._life,Float32Array], ["size",this._size,Float32Array],
                ["startRotation",this._startRotation,Float32Array],["startColor",this._startColor,Float32Array],
                ["startOpacity",this._startOpacity,Float32Array],["timeRotation",this._timeRotation,Float32Array],
                ["initDirection",this._initDirection,Float32Array],["direction",this._direction,Float32Array],
                ["textureRandomIndex",this._textureRandomIndex,Float32Array]
            ];
    ...
    updatePosition(index,delta){
        ...
        this._direction[index3] += this._force[index3] * delta;
        this._direction[index3_1] += this._force[index3_1] * delta;
        this._direction[index3_2] += this._force[index3_2] * delta;

        this._position[index3] += this._direction[index3] * delta;
        this._position[index3_1] += this._direction[index3_1] * delta;
        this._position[index3_2] += this._direction[index3_2] * delta;
    }
    ...
    bornAttribute(){
        ...
        this._position.unshift( shapeInfo.pos.x, shapeInfo.pos.y, shapeInfo.pos.z);
        ...
        this._startRotation.unshift(CoolMath.randomFloat(...this.startRotation));
        this._timeRotation.unshift(CoolMath.randomFloat(...this.timeRotation));
    ...
    set startRotation(startRotation){
        this._paras.startRotation = startRotation || [0,0];
    }
    ...
    set material(material){
        ...
        this._paras.material = material || new ThreeLibs.PointParticleMaterial({});
        this.material.isLocal = this.isLocal;
        this.material.algin = this.algin;
        this.material.timeSize = this.timeSize;
        this.material.pivotOffset = this.pivotOffset;
    ...
    get pivotOffset(){
        return this.renderMode.pivotOffset;
    }
    get algin(){
        const alginValue = {"view":0,"init":1,"speed":2};
    ...
    set timeRotation(timeRotation){
        this._paras.timeRotation = timeRotation || [0 , 0];
    }
    ...
    stop(){
        ...
        this._time= [];this._life = [];this._position = [];this._size = [];
        this._startRotation = [];this._initDirection = [];this._direction = [];
        this._force = [];this._timeRotation = [];this._startColor = [];
        this._startOpacity = [];this._textureRandomIndex = [];this._isUpdateAttribute = false;
    }

    //----------重写父组件方法----------
    clone(recursive) {
        let clone = new PointPaticle(this._copyParas(this._paras));
    ...
    _copyParas(paras){
        let _paras = {};
        [
            "duration","autoDispose","maxCount","pivotOffset","gravity","simulationSpace"
        ].forEach((key)=>{
            _paras[key] = paras[key];
        });
        [
            "startSize","startLife","startSpeed","startRotation","textureAnimation",
            "emission","force","startColor","startOpacity", "renderMode","timeSize",
            "timeRotation","timeColor","timeOpacity"
        ].forEach((key)=>{
